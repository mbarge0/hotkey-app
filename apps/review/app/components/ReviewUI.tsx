...
export default function ReviewUI({ dataUrl, appTitle = 'HotKey' }: ReviewUIProps) {
  ...
  async function loadBatch() {
    ...
    try {
      ...
      if (data.batches) {
        ...
        setBatchData({ batches: [data], currentIndex: 0, total: 1 })
      }
    } catch (err) {
      console.error('Error loading batch:', err)
      setBatchData({ batches: [], currentIndex: 0, total: 0 })
    }
    setLoading(false)
  }
  ...
  function toggleFormat(id: string) {
    ...
    const batchIdx = batchData.batches.indexOf(filteredBatches![currentIndex])
    updatedBatches[batchIdx] = {
      ...batch,
      formats: batch.formats.map(f => f.id === id ? { ...f, checked: !f.checked } : f)
    }
  ...
  function approveFormat(id: string) {
    ...
    updatedBatches[batchIdx] = {
      ...batch,
      formats: batch.formats.map(f => f.id === id ? { ...f, checked: true } : f)
    }
    setBatchData({ ...batchData, batches: updatedBatches })
  ...
  function updateScheduleTime(id: string, time: string) {
    ...
    updatedBatches[batchIdx] = {
      ...batch,
      formats: batch.formats.map(f => f.id === id ? { ...f, scheduleTime: time } : f)
    }
    setBatchData({ ...batchData, batches: updatedBatches })
  }
  ...
  function handleDiscardUnselected() {
    ...
    if (confirm(`Discard ${unselected.length} unselected formats?`)) {
      const updatedBatches = [...batchData.batches]
      const batchIdx = batchData.batches.indexOf(filteredBatches![currentIndex])
      updatedBatches[batchIdx] = { ...batch, formats: batch.formats.filter(f => f.checked) }
      setBatchData({ ...batchData, batches: updatedBatches })
    }
  }
  ...
  if (!batch) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center flex-col gap-4">
        <div className="text-xl text-gray-600">No content to review</div>
        <div className="text-sm text-gray-400">
          Run <code className="bg-gray-100 px-2 py-1 rounded">factory review-export</code> to populate this view
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => {
                  ...
                  updatedBatches[batchIdx] = {
                    ...batch,
                    formats: batch.formats.map(f => topIds.includes(f.id) ? { ...f, checked: true } : f)
                  }
                  setBatchData({ ...batchData, batches: updatedBatches })
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      {format.id === 'twitter' && <TwitterPreview content={format.content} media={format.media} />}
                      {format.id === 'linkedin' && <LinkedInPreview content={format.content} media={format.media} />}
                      {format.id === 'instagram' && <InstagramPreview content={format.content} media={format.media} />}
                    </div>

                    <div className={`border-t p-4 ${isScheduledOrPosted ? 'border-gray-300 bg-gray-100' : 'border-gray-200 bg-gray-50'}`}>
                      <div className="flex items-center justify-between mb-3">
                        ...
                        {isScheduledOrPosted && (
                          <button
                            onClick={(e) => { e.stopPropagation(); updateFormatStatus(format.id, 'unscheduled') }}
                      ...
                      {!isScheduledOrPosted && (
                        <>
                          <button
                            onClick={(e) => { e.stopPropagation(); approveFormat(format.id) }}
                          ...
                          <div className="flex items-center gap-2 mt-3">
                            <button
                              onClick={(e) => { e.stopPropagation(); updateFormatStatus(format.id, 'scheduled') }}
                              className="flex-1 px-3 py-1.5 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded text-xs font-medium transition-colors"
                            >
                              Already scheduled
                            </button>
                            <button
                              onClick={(e) => { e.stopPropagation(); updateFormatStatus(format.id, 'posted') }}
                              className="flex-1 px-3 py-1.5 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded text-xs font-medium transition-colors"
                            >
                              Already posted
                            </button>
                  ...
                  {otherFormats.map(format => (
                    ...
                    >
                      <div className="p-4">
                        ...
                        <div className="text-sm text-gray-700 line-clamp-3 mb-3">{format.content}</div>
                        <div className="flex gap-2">
                          <button
                            onClick={() => setSelectedFormat(format)}
                  ...
                  return (
                    <>
                      ...
                      <div className="space-y-2 text-sm text-gray-600">
                        {context.platformNotes.map((note, i) => <p key={i}>{note}</p>)}
                      </div>
                    </>
                  )
                })()}
      ...
      {selectedFormat && (
        <div
          ...
          onClick={() => { setSelectedFormat(null); setIsEditing(false) }}
        >
          <div
          ...
          >
            <div className="p-6 border-b border-gray-200 flex items-center justify-between bg-gradient-to-r from-blue-50 to-purple-50">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                    selectedFormat.publishType === 'auto' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                  }`}>
            ...
            <div className="p-6 overflow-y-auto" style={{ maxHeight: 'calc(90vh - 300px)' }}>
              {isEditing ? (
                ...
                <pre
                  ...
                  onClick={() => { setIsEditing(true); setEditedContent(selectedFormat.content) }}
                >
            ...
            <div className="p-6 border-t border-gray-200 bg-gray-50">
              ...
              <div className="flex justify-between gap-3">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    onClick={() => alert('Post Now - Coming soon with Publer integration!')}
                    className="px-6 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-lg font-medium transition-colors"
                  >
                    Post Now
                  </button>
                  <button
                    onClick={() => { approveFormat(selectedFormat.id); setSelectedFormat(null); setIsEditing(false) }}
